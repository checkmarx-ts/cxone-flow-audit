...
class AdoBase:
    ...
    @staticmethod
    def _update_hook_by_event_type(
    ...
    async def _list_lu_webhook_subscriptions(
        self,
        scm_base_url: str,
        collection: str,
        scm_pat: str,
        ...
        service_hooks_url = (
            f"{self._org_url(scm_base_url, collection)}/_apis/hooks/subscriptions"
        )
        service_params = {
        ...
        resp = await to_thread(
            ...
            headers=self._required_headers(scm_pat),
            proxies=proxies,
        ...
        if not resp.ok:
            self.log().error(
                f"Response of {resp.status_code} trying to retrieve service hook data from: {service_hooks_url}"
            )
        else:
            async with self.__lock:
    ...
    async def _lu_iterator_delegate(
        self,
        scm_base_url: str,
        targets: List[str],
        scm_pat: str,
        ...
        for t in targets:
            ...
            while True:
                ...
                resp = await to_thread(
                    ...
                    "GET",
                    project_list_url,
                    params=self.__project_list_url_params(skip),
                    headers=self._required_headers(scm_pat),
                    proxies=proxies,
                    verify=not ignore_ssl_errors,
                )
