...
class AdoDeployer(Deployer, AdoServiceManager):
    ...
    async def _process_lu(self, lu: Any) -> bool:
        subs = self._get_subs_for_project(
        ...
        if (
            await self._evaluate_subscription_state(hook_data) == ConfigState.CONFIGURED
            and not self.replace
        ):
            self.log().warning(
                f"{self._get_lu_repr(lu)} is already configured, no changes made."
            )
            return True
