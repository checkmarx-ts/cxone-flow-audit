...
class Operation:
  ...
  def __init__(self, targets : List[str], concurrency : Semaphore, match : NameMatcher, scm_api_service : SCMAPIService, cxoneflow_url : str):
  ...
  async def __thread(self, lu : Any) -> bool:
    async with self.__concurrency:
      if self.__match.matches(self.__scm_service.get_lu_name(lu)):
        ...
        self.log().info(f"LU skipped due to match rules: {self.__scm_service.get_lu_repr(lu)}")
        return True
