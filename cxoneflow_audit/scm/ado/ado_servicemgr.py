...
class AdoServiceManager(AdoBase):
    ...
    async def _create_subscription(
        ...
        if not resp.ok:
            self.log().error(
                f"Response code {resp.status_code} returned for {create_url}"
            )
    ...
    async def _delete_subscription(
        ...
        if not resp.ok:
            self.log().error(
                f"Response code {resp.status_code} returned for {delete_url}"
            )
            raise ScmException(
                f"Exception trying to delete service hook subscription {sub_id} from {collection}"
            )
