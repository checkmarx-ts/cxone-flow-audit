...
class AdoKicker(Kicker, AdoBase):
    ...
    async def __kickoff_msg_factory(
        self, collection_name: str, project_name: str, lu_repr: str, repo: Dict
    ) -> Tuple[Union[None, AdoKickoffMsg], Union[str, None]]:
        repo_name = None
        ...
        if len(clone_urls) == 0:
            self.log().warning(
                f"Repository {repo_name} had no clone URLs in LU {lu_repr}"
            )
        ...
        if len(AdoKicker.__repo_default_branch.find(repo)) > 0:
            default_branch = (
                AdoKicker.__repo_default_branch.find(repo)
                .pop()
                .value.replace("refs/heads/", "")
            )
        else:
            self.log().warning(
                f"Repository {repo_name} has no default branch in LU {lu_repr}"
            )
            return None, remote_url
        ...
        if not ref_list.ok:
            self.log().warning(
                f"{ref_list.status_code} returned when attempting to retrieve the ref list for repo {repo_name} in LU {lu_repr}"
            )
            return None, remote_url
        ...
        if len(AdoKicker.__ref_sha.find(ref_list_json)) > 0:
            ...
            self.log().warning(
                f"Repository {repo_name} default branch {default_branch} had no head commit SHA in LU {lu_repr}"
            )
            return None, remote_url
