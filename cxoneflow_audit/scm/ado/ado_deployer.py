...
class AdoDeployer(Deployer):
    ...
    async def _process_lu(self, lu: Any) -> bool:
        ...
        if (
            await self.scm_service.evaluate_subscription_state(hook_data) == ConfigState.CONFIGURED
        ...
        if len(sub_ids) > 0:
            await gather(*[self.scm_service.delete_subscription(lu["collection"],sub_id) for sub_id in sub_ids])

        await gather(*[self.scm_service.create_subscription(event,lu["id"],lu["collection"],self.cxoneflow_url,self.shared_secret) for event in ADOService.ADO_EVENT_TYPES])
