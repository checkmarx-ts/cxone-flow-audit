...
class SCMAPIService:
  def __init__(self, api_base_url : str, *args, proxy : str = None, ssl_verify : bool = True, **kwargs):
  ...
  async def _scm_api_call(self, api_path : str, headers : Dict = {}, query_args : Dict[str, str] = None, method : str = "GET", json_body : Dict = None, **kwargs) -> requests.Response:
    ...
    resp = await to_thread(requests.request, method, url, params=query_args,
                     ...
                     verify=self.__ssl_verify, json=json_body, **kwargs)
    
    if not resp.ok:
      raise ScmException(f"{url} response: {resp.status_code}: {resp.text}")
