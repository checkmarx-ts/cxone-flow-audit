...
from cxoneflow_kickoff_api import (
    KickoffClient,
    KickoffMsg,
    KickoffClientException,
    KickoffStatusCodes,
    KickoffResponseMsg,
)
...
class Kicker(Operation):
    def __init__(
        ...
        **kwargs,
        ...
        self.__ko_client = KickoffClient(
            key_bytes,
            ssh_private_key_password,
            self.cxoneflow_url.rstrip("/") + f"/{self.scm_key}/kickoff",
            PROGNAME,
            self.scm_service.proxies,
            self.scm_service.ssl_verify,
        )
    ...
    def __callback(
        self, status: KickoffStatusCodes, msg: KickoffResponseMsg, delay: int
    ) -> bool:
        if status == KickoffStatusCodes.TOO_MANY_SCANS:
            self.log().info(
                f"Server has {len(msg.running_scans)} running scans, asked to back off.  Delaying {delay} seconds before trying to start scan."
            )
            for scan in msg.running_scans:
                self.log().info(
                    f"Running: {scan.project_name}@{scan.scan_branch} (Scan Id: {scan.scan_id})"
                )
        else:
    ...
    async def _exec_kickoff(
        self, repo_repr: str, msg: KickoffMsg
    ) -> Union[KickoffStatusCodes, None]:
        # Throttle scan submissions to avoid the chance of overloading the system with scans.
        async with self.__lock:
            try:
                status, response = await self.__ko_client.kickoff_scan(
                    msg, self.__callback
                )
                if status == KickoffStatusCodes.SCAN_STARTED:
                    self.log().info(
                        f"Scan started: ScanId [{response.started_scan.scan_id}] Project [{response.started_scan.project_name}] for repository [{repo_repr}]"
                    )
                    await asyncio.sleep(2)
                    await self.__write_audit(
                        repo_repr, status, response.started_scan.scan_id
                    )
                    return status
                else:
                    self.log().warning(
                        f"Server responded with status [{status}] for scan attempt with repository [{repo_repr}]"
                    )
